...
public interface ScanRepository extends JpaRepository<Scan, Long> {
	...
	List<Scan> findAllCorrectScansInMonth(int year, int month);

	@Query(value = "select * from scan s where"
			+ " s.year = ?1", nativeQuery = true)
	List<Scan> findAllCorrectScansInYear(int year);
	...
	@Query(value = "select ROW_NUMBER() over (order by count(s.id) desc) as id, v.author, v.title, count(s.id) as amount" +
